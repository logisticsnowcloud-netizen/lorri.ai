      ...
      <div style={{ position: "relative", zIndex: 2, display: "flex", gap: 32, maxWidth: 1200, width: "100%", alignItems: "flex-start", flexWrap: "wrap" }}>
        ...
        <div style={{ flex: "1 1 280px", minWidth: 0, display: "flex", flexDirection: "column", alignItems: "flex-start" }}>
          ...
          <div className="fu2" style={{ width: "100%", maxWidth: 540, position: "relative" }}>
            ...
            {focused && suggestions.length > 0 && (
              <div style={{ position: "absolute", top: "calc(100% + 8px)", left: 0, right: 0, background: "var(--card)", border: "1px solid var(--border)", borderRadius: 12, overflow: "hidden", zIndex: 20, boxShadow: "0 20px 60px var(--shadow)", maxHeight: 300, overflowY: "auto" }}>
                ...
                {suggestions.map((s, i) => (
                  <div key={i} onClick={() => { setQuery(s.name); setFocused(false); navigate(`/map?location=${encodeURIComponent(s.name)}&lat=${s.lat}&lon=${s.lon}`); }}
          ...
          <div className="fu5" style={{ display: "flex", gap: 10, marginTop: 24, flexWrap: "wrap", alignItems: "center" }}>
            ...
            <button onClick={() => navigate("/global-grid")} style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "11px 22px", borderRadius: 10, border: "1px solid var(--border)", background: "var(--card)", color: "var(--text)", fontFamily: "Outfit,sans-serif", fontSize: 13, fontWeight: 600, cursor: "pointer", transition: "all .25s" }}>
              <GlobeNetIcon />
