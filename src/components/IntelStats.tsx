...
export default function IntelStats() {
  ...
  return (
    <section id="intelligence" ref={ref as any} className="relative overflow-hidden px-4 py-4 sm:px-6 lg:px-8" style={{ background: "hsl(var(--bg2))" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", position: "relative" }}>
        <div style={{ textAlign: "center", marginBottom: 14 }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "4px 12px", borderRadius: 20, fontSize: 10, fontWeight: 700, letterSpacing: ".08em", textTransform: "uppercase", background: "hsl(var(--primary) / 0.12)", border: "1px solid hsl(var(--border))", color: "hsl(var(--l-blue))", marginBottom: 8 }}>By The Numbers</div>
          <h2 style={{ fontSize: "1.6rem", fontWeight: 900, color: "hsl(var(--foreground))", letterSpacing: "-0.03em", lineHeight: 1.1, marginBottom: 8 }} className="text-[1.4rem] sm:text-[1.5rem] lg:text-[1.6rem]">
            National Scale,<br /><span style={{ color: "hsl(var(--primary))" }}>Real Intelligence</span>
          </h2>
        </div>
        <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
          {[
            ...
            <div key={i} style={{ padding: "20px 14px", textAlign: "center", background: `linear-gradient(160deg,${s.c}0A 0%,transparent 60%)`, border: `1px solid ${s.c}22`, borderRadius: 12, animation: visible ? `fadeUp .7s ${i * 0.1}s ease both` : "none", opacity: visible ? undefined : 0 }}>
              ...
              <div style={{ fontSize: 13, color: "hsl(var(--muted-foreground))", lineHeight: 1.5 }}>{s.l}</div>
            </div>
          ))}
        </div>
      </div>
