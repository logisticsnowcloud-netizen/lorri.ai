...
export default function CTASection() {
  ...
  return (
    <section id="cta" className="relative overflow-hidden px-4 py-4 sm:px-6 lg:px-8" style={{ background: "var(--bg2)" }}>
        ...
        <div className="grid gap-8 xl:grid-cols-2 xl:items-center">
          <div>
            ...
            <h2 style={{ fontSize: "1.6rem", fontWeight: 900, color: "var(--text)", letterSpacing: "-0.03em", lineHeight: 1.1, marginBottom: 10 }} className="text-[1.4rem] sm:text-[1.5rem] lg:text-[1.6rem]">
              Experience the<br />
              <span style={{ color: "#393185" }}>LoRRI Difference</span>
            </h2>
            ...
            <div style={{ padding: "18px 22px", background: "var(--card)", border: "1px solid var(--borderSm)", borderRadius: 14 }}>
              ...
              <div style={{ fontSize: 14, color: "#1AA6DF", marginBottom: 4, overflowWrap: "anywhere" }}>lorri@logisticsnow.in</div>
              <div style={{ fontSize: 14, color: "var(--text2)", marginBottom: 5 }}>+91-9867773508</div>
              <div style={{ fontSize: 13, color: "var(--text3)", lineHeight: 1.6 }}>409, Neptune's Flying Colors, Mulund West, Mumbai 400080</div>
            </div>
          </div>

          <div style={{ background: "var(--card)", border: "1.5px solid var(--border)", borderRadius: 16, padding: 18 }} className="sm:p-[22px]">
            {sent ? (
              ...
              <>
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2" style={{ marginBottom: 16 }}>
                  {[{ v: "shipper", l: "🏭 Shipper", c: "#393185", d: "var(--purpleLt)" }, { v: "carrier", l: "🚛 Carrier", c: "#54AF3A", d: "var(--greenLt)" }].map((b) => (
                    <button key={b.v} onClick={() => set("type", b.v)} style={{ padding: 10, borderRadius: 9, border: `1.5px solid ${form.type === b.v ? b.c : "var(--border)"}`, background: form.type === b.v ? b.d : "transparent", color: form.type === b.v ? "var(--text)" : "var(--text2)", fontFamily: "Outfit,sans-serif", fontSize: 13, fontWeight: 600, cursor: "pointer", transition: "all .2s" }}>{b.l}</button>
                  ))}
                </div>
                {[{ k: "name", p: "Your full name", l: "Full Name" }, { k: "company", p: "Your company", l: "Company" }, { k: "email", p: "you@company.com", l: "Email" }, { k: "phone", p: "+91 XXXXX XXXXX", l: "Phone" }].map((f) => (
                  <div key={f.k} style={{ marginBottom: 12 }}>
                    ...
                    <input value={form[f.k as keyof typeof form]} onChange={(e) => set(f.k, e.target.value)} placeholder={f.p} style={{ width: "100%", background: "var(--inputBg)", border: "1px solid var(--border)", borderRadius: 9, padding: "11px 14px", color: "var(--text)", fontFamily: "Outfit,sans-serif", fontSize: 14, outline: "none" }} />
                  </div>
                ))}
                <button onClick={() => { if (form.name && form.email) setSent(true); }} style={{ width: "100%", display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 8, background: "linear-gradient(135deg,#393185,#4D44A8)", color: "#fff", border: "none", padding: 14, borderRadius: 8, fontFamily: "Outfit,sans-serif", fontSize: 15, fontWeight: 700, cursor: "pointer", letterSpacing: ".05em", textTransform: "uppercase", boxShadow: "0 4px 20px rgba(57,49,133,0.4)", marginTop: 4 }}>
